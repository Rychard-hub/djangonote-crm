...
describe("TriggerClient", () => {
    ...
    it("fills missing fields from env, but explicit constructor values still win", async () => {
        ...
        const byRun = Object.fromEntries(fetchSpy.captured.map((r) => [r.url.split("/runs/")[1]?.split(/[/?]/)[0], r]));
    ...
    it("keeps two concurrent instances isolated from each other", async () => {
        ...
        const byPath = Object.fromEntries(fetchSpy.captured.map((r) => [r.url.split("/runs/")[1]?.split(/[/?]/)[0], r]));
    ...
    it("masks taskContext.ctx inside an isolated scope (default)", () => {
        ...
        taskContext.setGlobalTaskContext({ ctx: fakeCtx });
        expect(taskContext.ctx).toBe(fakeCtx);
        const observed = sdkScope.withScope({ apiClientConfig: { accessToken: "x" }, inheritContext: false }, () => taskContext.ctx);
    ...
    it("exposes taskContext.ctx inside a scope when inheritContext is true", () => {
        ...
        taskContext.setGlobalTaskContext({ ctx: fakeCtx });
        const observed = sdkScope.withScope({ apiClientConfig: { accessToken: "x" }, inheritContext: true }, () => taskContext.ctx);
...
describe("auth.withAuth", () => {
    ...
    it("inherits TRIGGER_SECRET_KEY from env when called with a partial config", async () => {
        ...
        expect(await auth.withAuth({ baseURL: "https://override.example.com" }, async () => apiClientManager.baseURL)).toBe("https://override.example.com");
    });
    ...
    it("does not stomp on a parallel withAuth call with a different config", async () => {
        ...
        const gateA = new Promise((r) => (settle.resolveA = r));
        const gateB = new Promise((r) => (settle.resolveB = r));
...
//# sourceMappingURL=triggerClient.test.js.map