...
describe("TriggerChatTransport", () => {
    ...
    describe("sendMessages", () => {
        ...
        it("functional baseURL dispatches per endpoint (in vs out)", async () => {
            ...
            const baseURLFn = vi.fn(({ endpoint }) => endpoint === "out" ? "https://stream.example.com" : "https://api.example.com");
            const transport = new TriggerChatTransport({
                task: "my-chat-task",
                accessToken: () => "pat",
                baseURL: baseURLFn,
    ...
    describe("sendAction", () => {
        ...
        it("marks the session streaming and notifies before subscribing", async () => {
            ...
            expect(onSessionChange.mock.calls.some(([, session]) => session && session.isStreaming === true)).toBe(true);
            await drainChunks(stream);
        });
    });
