...
const debug = (0, debug_1.default)("s2:fetch");
class FetchReadSession extends ReadableStream {
    static async create(client, name, args, options) {
        ...
        if (response.error) {
            ...
            const error = typeof response.error === "object" &&
                ...
                : status === 416
                    ? new error_js_1.RangeNotSatisfiableError({
                        status,
                    })
                    : new error_js_1.S2Error({
    ...
    constructor(stream, format) {
        ...
        const eventStream = new event_stream_js_1.EventStream(stream, (msg) => {
            ...
            if (msg.event === "error") {
                ...
                parserError = new error_js_1.S2Error({
            ...
            pull: async (controller) => {
                ...
                while (true) {
                    ...
                    try {
                        ...
                        controller.enqueue({ ok: false, error: (0, error_js_1.s2Error)(error) });
                        done = true;
                        await reader.cancel();
                        controller.close();
                        return;
...
class FetchAppendSession {
    ...
    async close() {
        ...
        try {
            ...
            const s2Err = (0, error_js_1.s2Error)(error);
    ...
    submit(input) {
        ...
        if (input.records.length > 1000) {
            ...
            return Promise.resolve((0, result_js_1.err)(new error_js_1.S2Error({
    ...
    async processLoop() {
        ...
        while (this.queue.length > 0) {
            ...
            try {
                const preferProtobuf = input.records.some((record) => (0, utils_js_1.computeAppendRecordFormat)(record) === "bytes");
                const appendOptions = this.options
                    ? { ...this.options, preferProtobuf }
                    : { preferProtobuf };
                const ack = await (0, shared_js_1.streamAppend)(this.stream, this.client, input, appendOptions);
                ...
                resolver.resolve((0, result_js_1.ok)(ack));
            }
            catch (error) {
                ...
                resolver.resolve((0, result_js_1.err)(s2Err));
                ...
                for (const pendingResolver of this.pendingResolvers) {
                    pendingResolver.resolve((0, result_js_1.err)(s2Err));
                }
