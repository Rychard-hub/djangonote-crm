...
function installStubApiClient(impl) {
    const stub = { initializeSessionStream: impl };
    // `apiClientManager.clientOrThrow()` is what `#pipeInternal` reaches for.
    vitest_1.vi.spyOn(v3_1.apiClientManager, "clientOrThrow").mockReturnValue(stub);
    return stub;
}
function emptyStream() {
    return new ReadableStream({ start: (c) => c.close() });
}
(0, vitest_1.describe)("SessionOutputChannel initializeSessionStream cache", () => {
    (0, vitest_1.it)("dedupes repeated pipe()/writer() calls for the same channel", async () => {
        stubWaitImpl = undefined;
        const spy = vitest_1.vi.fn(async () => ({ version: "v2", headers: {} }));
        installStubApiClient(spy);
        const channel = new sessions_js_1.SessionOutputChannel("session-1");
        ...
        (0, vitest_1.expect)(spy).toHaveBeenCalledWith("session-1", "out", undefined);
    });
    (0, vitest_1.it)("evicts on initialize failure so the next call retries instead of returning a poisoned entry", async () => {
        stubWaitImpl = undefined;
    ...
    (0, vitest_1.it)("reset() clears cached entries so the next call re-PUTs", async () => {
        stubWaitImpl = undefined;
        const spy = vitest_1.vi.fn(async () => ({ version: "v2", headers: {} }));
        installStubApiClient(spy);
        const channel = new sessions_js_1.SessionOutputChannel("session-1");
    ...
    (0, vitest_1.it)("scopes the cache per channel instance", async () => {
        stubWaitImpl = undefined;
        const spy = vitest_1.vi.fn(async () => ({ version: "v2", headers: {} }));
        installStubApiClient(spy);
        const channelA = new sessions_js_1.SessionOutputChannel("session-a");
        const channelB = new sessions_js_1.SessionOutputChannel("session-b");
        await Promise.all([
            channelA.pipe(emptyStream()).waitUntilComplete(),
            channelB.pipe(emptyStream()).waitUntilComplete(),
        ]);
        (0, vitest_1.expect)(spy).toHaveBeenCalledTimes(2);
        (0, vitest_1.expect)(spy).toHaveBeenCalledWith("session-a", "out", undefined);
        (0, vitest_1.expect)(spy).toHaveBeenCalledWith("session-b", "out", undefined);
    });
    (0, vitest_1.it)("evicts the cache when a writer's wait() rejects (simulated stale-token failure)", async () => {
        const spy = vitest_1.vi.fn(async () => ({ version: "v2", headers: {} }));
        installStubApiClient(spy);
        ...
        stubWaitImpl = async () => {
            waitCallCount++;
            if (waitCallCount === 1)
                throw new Error("S2 auth failed: token expired");
            return { lastEventId: undefined };
        };
        const channel = new sessions_js_1.SessionOutputChannel("session-1");
        const failed = channel.pipe(emptyStream());
        await (0, vitest_1.expect)(failed.waitUntilComplete()).rejects.toThrow(/token expired/);
