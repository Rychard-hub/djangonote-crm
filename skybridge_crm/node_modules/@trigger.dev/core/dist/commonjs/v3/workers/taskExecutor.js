...
class TaskExecutor {
    ...
    async execute(execution, ctx, signal) {
        ...
        const result = await this._tracer.startActiveSpan(attemptMessage, async (span) => {
            ...
            events: execution.attempt.number === 1
                ...
                : undefined,
        }, index_js_1.traceContext.extractContext());
        return { result };
    }
    ...
    async #callOnWaitFunctions(wait, payload, ctx, initOutput, signal) {
        ...
        const _result = await run_timeline_metrics_api_js_1.runTimelineMetrics.measureMetric("trigger.dev/execution", "onWait", async () => {
    ...
    async #callOnCancelFunctions(runPromise, payload, ctx, initOutput, signal, attemptContext) {
        ...
        const _result = await run_timeline_metrics_api_js_1.runTimelineMetrics.measureMetric("trigger.dev/execution", "onCancel", async () => {
    ...
    async #callOnResumeFunctions(wait, payload, ctx, initOutput, signal) {
        ...
        const _result = await run_timeline_metrics_api_js_1.runTimelineMetrics.measureMetric("trigger.dev/execution", "onResume", async () => {
    ...
    async #cleanupAndWaitUntil(payload, ctx, initOutput, signal) {
        ...
        index_js_1.inputStreams.clearHandlers();
        index_js_1.sessionStreams.clearHandlers();
