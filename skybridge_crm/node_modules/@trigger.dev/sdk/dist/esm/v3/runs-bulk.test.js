...
describe("runs.bulk", () => {
    ...
    it("creates a cancel bulk action", async () => {
        ...
        const result = await withApiClient(() => runs.bulk.cancel({ runIds: ["run_1", "run_2"], name: "Cancel selected" }));
    ...
    it("creates a replay bulk action", async () => {
        ...
        const result = await withApiClient(() => runs.bulk.replay({
    ...
    it("polls until the bulk action finishes", async () => {
        ...
        const bulkAction = await withApiClient(() => runs.bulk.poll("bulk_poll", { pollIntervalMs: 1 }));
    ...
    function withApiClient(fn) {
        return apiClientManager.runWithConfig({ baseURL: baseUrl, accessToken: "tr_test_key" }, async () => fn());
    }
});
