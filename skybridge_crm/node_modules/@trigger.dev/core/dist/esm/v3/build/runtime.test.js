...
describe("runtime configuration", () => {
    it.each(["node", "node-22", "experimental-node-24", "experimental-node-26", "bun"])("accepts %s as a public config runtime", (runtime) => {
    ...
    it.each([
        ...
        ["node-22", "node-22"],
        ["bun", "bun"],
    ])("normalizes %s to %s", (runtime, expected) => {
        expect(resolveBuildRuntime(runtime)).toBe(expected);
    });
    it.each(["node-24", "node-26"])("keeps internal runtime %s out of the public config schema", (runtime) => {
    ...
    it("rejects unsupported config runtimes with a clear error", () => {
        expect(() => resolveBuildRuntime("node-23")).toThrowError(/Unsupported runtime "node-23" in trigger\.config\. Supported runtimes:/);
    });
    it.each(["experimental-node-24", "experimental-node-26"])("keeps %s out of internal runtime schemas", (runtime) => {
