...
(0, vitest_1.describe)("RunQueueConsumerPool", () => {
    ...
    (0, vitest_1.describe)("Consumer lifecycle management", () => {
        ...
        (0, vitest_1.it)("should forward dequeue messages with queue length updates", async () => {
            ...
            (0, vitest_1.expect)(mockOnDequeue).toHaveBeenCalledWith(messages, undefined);
            advanceTimeAndProcessMetrics(1100);
            const metrics = pool.getMetrics();
    ...
    (0, vitest_1.describe)("Metrics wiring", () => {
        (0, vitest_1.it)("injects the pool's shared ConsumerPoolMetrics into every consumer when a registry is provided", async () => {
            pool = new consumerPool_js_1.RunQueueConsumerPool({
                ...defaultOptions,
                metricsRegistry: new prom_client_1.Registry(),
                scaling: { strategy: "none", maxConsumerCount: 3 },
            });
            await pool.start();
            (0, vitest_1.expect)(testConsumers.length).toBe(3);
            const poolMetrics = pool["promMetrics"];
            (0, vitest_1.expect)(poolMetrics).toBeInstanceOf(consumerPoolMetrics_js_1.ConsumerPoolMetrics);
            testConsumers.forEach((consumer) => {
                (0, vitest_1.expect)(consumer.metrics).toBe(poolMetrics);
            });
        });
        (0, vitest_1.it)("preserves a caller-supplied consumer metrics instance when no registry is provided", async () => {
            const callerMetrics = new consumerPoolMetrics_js_1.ConsumerPoolMetrics({ register: new prom_client_1.Registry() });
            pool = new consumerPool_js_1.RunQueueConsumerPool({
                ...defaultOptions,
            ...
            await pool.start();
            (0, vitest_1.expect)(testConsumers[0]?.metrics).toBe(callerMetrics);
        });
    });
    (0, vitest_1.describe)("Backpressure scale-up freeze", () => {
        (0, vitest_1.it)("freezes scale-up while shouldPauseScaling returns true, then resumes", async () => {
            let paused = true;
            pool = new consumerPool_js_1.RunQueueConsumerPool({
                ...defaultOptions,
                scaling: {
        ...
        (0, vitest_1.it)("still allows scale-down while paused", async () => {
            let paused = false;
            pool = new consumerPool_js_1.RunQueueConsumerPool({
                ...defaultOptions,
                scaling: {
            ...
            (0, vitest_1.expect)(scaledUp).toBeGreaterThan(1);
            ...
            (0, vitest_1.expect)(pool.size).toBeLessThan(scaledUp);
        });
    });
});
