...
exports.triggerAndSubscribe = triggerAndSubscribe;
...
exports.offloadBatchItemPayloads = offloadBatchItemPayloads;
...
function createTask(params) {
    const task = {
        ...
        triggerAndSubscribe: (payload, options) => {
            return new v3_1.TaskRunPromise((resolve, reject) => {
                triggerAndSubscribe_internal("triggerAndSubscribe()", params.id, payload, undefined, {
                    queue: params.queue?.name,
                    ...options,
                })
                    .then((result) => {
...
function createSchemaTask(params) {
    ...
    const task = {
        ...
        triggerAndSubscribe: (payload, options) => {
            return new v3_1.TaskRunPromise((resolve, reject) => {
                triggerAndSubscribe_internal("triggerAndSubscribe()", params.id, payload, parsePayload, {
                    queue: params.queue?.name,
                    ...options,
                })
                    .then((result) => {
...
function triggerAndSubscribe(id, payload, options, requestOptions) {
    return new v3_1.TaskRunPromise((resolve, reject) => {
        triggerAndSubscribe_internal("tasks.triggerAndSubscribe()", id, payload, undefined, options, requestOptions)
            .then((result) => {
            resolve(result);
        })
            .catch((error) => {
...
async function offloadBatchItemPayload(item, apiClient) {
    ...
    const { size: payloadSize } = (0, v3_1.packetRequiresOffloading)(packet);
    const exported = await (0, v3_1.conditionallyExportPacket)(packet, createTriggerPayloadPathPrefix(item.task), undefined, apiClient);
    return {
        ...item,
...
async function triggerAndSubscribe_internal(name, id, payload, parsePayload, options, requestOptions) {
    ...
    return await tracer_js_1.tracer.startActiveSpan(name, async (span) => {
        ...
        span.setAttribute("runId", response.id);
        span.setAttribute(v3_1.SemanticInternalAttributes.ENTITY_TYPE, "run");
        span.setAttribute(v3_1.SemanticInternalAttributes.ENTITY_ID, response.id);
...
async function prepareTriggerPayload(payload, apiClient, taskId) {
    ...
    const { size: payloadSize } = (0, v3_1.packetRequiresOffloading)(payloadPacket);
    const packet = await (0, v3_1.conditionallyExportPacket)(payloadPacket, createTriggerPayloadPathPrefix(taskId), undefined, apiClient);
