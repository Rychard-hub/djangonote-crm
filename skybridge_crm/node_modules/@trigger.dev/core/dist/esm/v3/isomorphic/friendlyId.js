...
export function regionCharForRegion(region) {
...
export function base32hexEncode(bytes) {
    let out = "";
    let buf = 0;
    let bits = 0;
...
export function base32hexDecode(s) {
    const out = [];
    let buf = 0;
    let bits = 0;
    for (const c of s) {
 ...
 */
export function generateRunOpsId(region) {
 ...
 */
export function parseRunOpsIdBody(body) {
    if (body.length !== RUN_OPS_ID_LENGTH)
        return undefined;
    if (body[RUN_OPS_ID_VERSION_INDEX] !== RUN_OPS_ID_VERSION)
        return undefined;
...
export function isRunOpsIdBody(body) {
