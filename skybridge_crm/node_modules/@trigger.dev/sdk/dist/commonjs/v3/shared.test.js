...
(0, vitest_1.describe)("offloadBatchItemPayloads", () => {
    ...
    const apiClient = new v3_1.ApiClient("http://localhost:3030", "tr_dev_test");
    (0, vitest_1.it)("returns an empty array unchanged", async () => {
        (0, vitest_1.expect)(await (0, shared_js_1.offloadBatchItemPayloads)([], apiClient)).toEqual([]);
    });
    (0, vitest_1.it)("passes small payloads through and records their pre-offload byte size", async () => {
        const payload = JSON.stringify({ hello: "world" });
        const result = await (0, shared_js_1.offloadBatchItemPayloads)([{ index: 0, task: "my-task", payload, options: { payloadType: "application/json" } }], apiClient);
        const item = result[0];
        (0, vitest_1.expect)(item.payload).toBe(payload);
        (0, vitest_1.expect)(item.options?.payloadType).toBe("application/json");
        (0, vitest_1.expect)(item.options?.payloadSize).toBe(Buffer.byteLength(payload, "utf8"));
    });
    (0, vitest_1.it)("measures multi-byte payloads by byte length, not character count", async () => {
        const payload = "€€€"; // 3 chars, 9 bytes in UTF-8
        const result = await (0, shared_js_1.offloadBatchItemPayloads)([{ index: 0, task: "my-task", payload, options: { payloadType: "application/json" } }], apiClient);
        (0, vitest_1.expect)(result[0].options?.payloadSize).toBe(9);
    });
    (0, vitest_1.it)("leaves an already-offloaded (application/store) item untouched", async () => {
        const item = {
            index: 0,
            task: "my-task",
        ...
        const result = await (0, shared_js_1.offloadBatchItemPayloads)([item], apiClient);
        (0, vitest_1.expect)(result[0]).toEqual(item);
    });
    (0, vitest_1.it)("leaves items without a string payload untouched", async () => {
