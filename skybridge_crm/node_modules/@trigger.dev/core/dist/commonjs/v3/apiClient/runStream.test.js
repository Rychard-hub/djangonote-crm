...
vitest_1.vi.setConfig({ testTimeout: 10_000 });
(0, vitest_1.describe)("SSEStreamSubscription retry behavior", () => {
    ...
    (0, vitest_1.it)("retries past the legacy 5-attempt cap", async () => {
        let attempts = 0;
        globalThis.fetch = vitest_1.vi.fn().mockImplementation(async () => {
            attempts++;
            if (attempts < 8) {
        ...
        const sub = new runStream_js_1.SSEStreamSubscription("http://example.test/sse", {
        ...
        (0, vitest_1.expect)(attempts).toBe(8);
        (0, vitest_1.expect)(result.error).toBeUndefined();
        (0, vitest_1.expect)(result.chunks).toHaveLength(1);
    });
    (0, vitest_1.it)("caps the exponential backoff at maxRetryDelayMs", async () => {
        let attempts = 0;
        const callTimes = [];
        globalThis.fetch = vitest_1.vi.fn().mockImplementation(async () => {
        ...
        const stream = await sub.subscribe();
        await drain(stream);
        (0, vitest_1.expect)(attempts).toBe(6);
        ...
        (0, vitest_1.expect)(totalElapsed).toBeLessThan(250);
    });
    (0, vitest_1.it)("retryNow() wakes an in-flight backoff and reconnects immediately", async () => {
        let attempts = 0;
        globalThis.fetch = vitest_1.vi.fn().mockImplementation(async () => {
            attempts++;
            if (attempts === 1) {
    ...
    (0, vitest_1.it)("respects abort signal during retry backoff", async () => {
        let attempts = 0;
        globalThis.fetch = vitest_1.vi.fn().mockImplementation(async () => {
            attempts++;
            throw new TypeError("fetch failed");
        ...
        (0, vitest_1.expect)(attempts).toBeLessThanOrEqual(2);
    });
    (0, vitest_1.it)("forceReconnect mid-read drops the stream and resumes with Last-Event-ID", async () => {
        let attempts = 0;
        const seenLastEventIds = [];
        globalThis.fetch = vitest_1.vi.fn().mockImplementation(async (_url, init) => {
            attempts++;
            const lastEventIdHeader = init?.headers?.["Last-Event-ID"];
        ...
        (0, vitest_1.expect)(first.value.chunk.first).toBe(true);
        sub.forceReconnect();
        // Second chunk arrives from the resumed connection.
        const second = await reader.read();
        (0, vitest_1.expect)(second.done).toBe(false);
        (0, vitest_1.expect)(second.value.chunk.second).toBe(true);
        const tail = await reader.read();
        (0, vitest_1.expect)(tail.done).toBe(true);
        (0, vitest_1.expect)(attempts).toBe(2);
        (0, vitest_1.expect)(seenLastEventIds[0]).toBeNull();
        // Resumed request includes the Last-Event-ID from the first chunk.
        (0, vitest_1.expect)(seenLastEventIds[1]).toBe("7");
    });
    (0, vitest_1.it)("forceReconnect aborts the in-flight fetch and retries", async () => {
        let attempts = 0;
        let firstResolve;
        globalThis.fetch = vitest_1.vi.fn().mockImplementation(async (_url, init) => {
            attempts++;
        ...
        (0, vitest_1.expect)(firstResolve).toBeDefined();
    });
    (0, vitest_1.it)("aborts a slow fetch via fetchTimeoutMs and retries", async () => {
        let attempts = 0;
        globalThis.fetch = vitest_1.vi.fn().mockImplementation(async (_url, init) => {
            attempts++;
            if (attempts === 1) {
    ...
    (0, vitest_1.it)("aborts a silent reader via stallTimeoutMs and retries", async () => {
        let attempts = 0;
        globalThis.fetch = vitest_1.vi.fn().mockImplementation(async (_url, init) => {
            attempts++;
            if (attempts === 1) {
    ...
    (0, vitest_1.it)("does not retry on 404 (stream gone)", async () => {
        let attempts = 0;
        globalThis.fetch = vitest_1.vi.fn().mockImplementation(async () => {
            attempts++;
    ...
    (0, vitest_1.it)("does not retry on 410 (session closed)", async () => {
        let attempts = 0;
        globalThis.fetch = vitest_1.vi.fn().mockImplementation(async () => {
            attempts++;
    ...
    (0, vitest_1.it)("respects custom nonRetryableStatuses", async () => {
        let attempts = 0;
        globalThis.fetch = vitest_1.vi.fn().mockImplementation(async () => {
            attempts++;
    ...
    (0, vitest_1.it)("retries on 503 (caller-tunable nonRetryableStatuses)", async () => {
        let attempts = 0;
        globalThis.fetch = vitest_1.vi.fn().mockImplementation(async () => {
            attempts++;
            if (attempts < 3)
    ...
    (0, vitest_1.it)("applies jitter to backoff (delays vary across attempts)", async () => {
        const callTimes = [];
        globalThis.fetch = vitest_1.vi.fn().mockImplementation(async () => {
        ...
        ac.abort();
        await promise;
        (0, vitest_1.expect)(callTimes.length).toBeGreaterThanOrEqual(5);
        ...
        (0, vitest_1.expect)(min).toBeGreaterThanOrEqual(20); // a little slack for timer scheduling
        (0, vitest_1.expect)(max).toBeLessThanOrEqual(80);
        ...
        (0, vitest_1.expect)(max - min).toBeGreaterThan(2);
    });
});
(0, vitest_1.describe)("SSEStreamSubscription v2 batch parsing — record kinds", () => {
    const originalFetch = globalThis.fetch;
    (0, vitest_1.afterEach)(() => {
        globalThis.fetch = originalFetch;
        vitest_1.vi.restoreAllMocks();
    ...
    function makeBatchResponse(records) {
        const body = new ReadableStream({
            start(controller) {
    ...
    (0, vitest_1.it)("data records flow through with headers and parsed body", async () => {
        globalThis.fetch = vitest_1.vi.fn().mockResolvedValue(makeBatchResponse([
            {
        ...
        const sub = new runStream_js_1.SSEStreamSubscription("http://x", { maxRetries: 0 });
        const parts = await sub.subscribe().then(drain);
        (0, vitest_1.expect)(parts).toHaveLength(1);
        (0, vitest_1.expect)(parts[0].id).toBe("5");
        (0, vitest_1.expect)(parts[0].chunk).toEqual({ type: "text-delta", delta: "hi" });
        (0, vitest_1.expect)(parts[0].headers).toEqual([]);
    });
    (0, vitest_1.it)("S2 command records (empty-name header) are filtered out", async () => {
        globalThis.fetch = vitest_1.vi.fn().mockResolvedValue(makeBatchResponse([
            {
        ...
        (0, vitest_1.expect)(parts).toHaveLength(2);
        (0, vitest_1.expect)(parts[0].chunk.delta).toBe("before");
        (0, vitest_1.expect)(parts[1].chunk.delta).toBe("after");
    });
    (0, vitest_1.it)("trigger-control records flow with headers and undefined chunk", async () => {
        globalThis.fetch = vitest_1.vi.fn().mockResolvedValue(makeBatchResponse([
            {
                body: "",
                seq_num: 7,
        ...
        (0, vitest_1.expect)(parts).toHaveLength(1);
        (0, vitest_1.expect)(parts[0].chunk).toBeUndefined();
        (0, vitest_1.expect)(parts[0].headers).toEqual([
            ["trigger-control", "turn-complete"],
            ["public-access-token", "eyJ..."],
        ]);
    });
    (0, vitest_1.it)("malformed data record body does not crash; cursor still advances", async () => {
        globalThis.fetch = vitest_1.vi.fn().mockResolvedValue(makeBatchResponse([
            {
        ...
        (0, vitest_1.expect)(parts).toHaveLength(2);
        (0, vitest_1.expect)(parts[0].chunk).toBeUndefined();
        (0, vitest_1.expect)(parts[1].chunk.delta).toBe("x");
    });
});
//# sourceMappingURL=runStream.test.js.map