...
exports.StandardSessionStreamManager = void 0;
const types_js_1 = require("../inputStreams/types.js");
const reconnectBackoff_js_1 = require("../utils/reconnectBackoff.js");
const wireProtocol_js_1 = require("./wireProtocol.js");
function keyFor(sessionId, io) {
    return `${sessionId}:${io}`;
}
/**
 ...
 * stream SSE.
 */
class StandardSessionStreamManager {
    apiClient;
    baseUrl;
    debug;
    handlers = new Map();
        ...
        if (buffered && buffered.length > 0) {
            ...
            for (let i = 0; i < buffered.length; i++) {
                ...
                if (consumed) {
                    const s = seqList[i];
                    if (s !== undefined)
                        this.#advanceLastDispatched(key, s);
                }
                else {
        ...
        return new types_js_1.InputStreamOncePromise((resolve, reject) => {
            ...
            if (options?.timeoutMs) {
                waiter.timeoutHandle = setTimeout(() => {
                    ...
                    resolve({
                        ok: false,
                        error: new types_js_1.InputStreamTimeoutError(key, options.timeoutMs),
                    });
                }, options.timeoutMs);
            }
    ...
    #advanceLastDispatched(key, seqNum) {
    ...
    #ensureTailConnected(sessionId, io) {
        const key = keyFor(sessionId, io);
        ...
        const promise = this.#runTail(sessionId, io, abortController.signal)
            ...
            if (hasHandlers || hasWaiters) {
                ...
                setTimeout(() => {
                    ...
                    const stillHasWaiters = this.onceWaiters.has(key) && this.onceWaiters.get(key).length > 0;
    ...
    async #runTail(sessionId, io, signal) {
        const key = keyFor(sessionId, io);
        try {
            ...
            const stream = await this.apiClient.subscribeToSessionStream(sessionId, io, {
                signal,
                ...
                onPart: (part) => {
                    ...
                    if ((0, wireProtocol_js_1.controlSubtype)(part.headers)) {
                        return;
                    }
...
exports.StandardSessionStreamManager = StandardSessionStreamManager;
