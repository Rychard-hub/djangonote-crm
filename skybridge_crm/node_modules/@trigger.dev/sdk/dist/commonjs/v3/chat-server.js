...
exports.chat = {
    ...
    headStart(opts) {
        ...
        if (apiClient) {
            return async (req) => v3_1.apiClientManager.runWithConfig(apiClient, () => handler(req));
        }
        return handler;
    },
    /**
     ...
     * ```
     */
    async startHeadStart(opts) {
        const open = () => openHandoverSession({
            chatId: opts.chatId,
        ...
        const session = opts.apiClient
            ? await v3_1.apiClientManager.runWithConfig(opts.apiClient, open)
            : await open();
        const helper = {
            toStreamTextOptions(spreadOpts) {
                return session.buildStreamTextOptions(spreadOpts);
 ...
 * messages directly.
 */
async function parseHandoverRequest(req) {
    const wirePayload = (await req.json());
...
async function openHandoverSession(opts) {
    ...
    const modelMessages = await (0, ai_runtime_js_1.convertToModelMessages)(uiMessages);
    const apiClient = resolveApiClient();
    // Top-level `idleTimeoutInSeconds` wins over the one in `triggerConfig`.
    const idleTimeoutInSeconds = opts.idleTimeoutInSeconds ?? opts.triggerConfig?.idleTimeoutInSeconds ?? 60;
    ...
    const buildStreamTextOptions = (spreadOpts) => {
        ...
        return {
            ...
            stopWhen: (0, ai_runtime_js_1.stepCountIs)(1),
    ...
    const drainToSessionOut = (stream) => {
        sessionWriter = new v3_1.SessionStreamInstance({
            apiClient,
            baseUrl: apiClient.baseUrl,
            sessionId: chatId,
    ...
    const turnMessageId = (0, ai_runtime_js_1.generateId)();
    let resolveDecision;
...
function toNodeListener(webHandler) {
    return async function nodeListener(req, res) {
        ...
        try {
            ...
            const setCookies = typeof webRes.headers.getSetCookie ===
                "function"
